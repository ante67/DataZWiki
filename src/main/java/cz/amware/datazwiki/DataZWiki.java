...
public class DataZWiki {
   ...
   Document document = Jsoup.connect
        ("https://en.wikipedia.org/wiki/List_of_IBA_official_cocktails")
           ...
           .userAgent
  ("Mozilla/5.0 (Windows NT 6.1;WOW64;rw23.0) Gecko/20100101 Firefox 23.0")
   ...
   for(int i=0;i<100; i++) {
       ...
       String attrHref = elementNew.getElementsByTag("a").get(0)
               .attr("href"); 
    ...
    for (Iterator<String> iterator = cocktailsUrlList.iterator();
            iterator.hasNext();){
 ...
 ("Mozilla/5.0 (Windows NT 6.1;WOW64;rw23.0) Gecko/20100101 Firefox 23.0")
           .maxBodySize(0).timeout(600000).get();
        String name = document2.getElementsByTag
             ("caption").get(0).text(); //COCKTAIL NAME
        TCocktail cocktailForCrossTable = insertCocktail(name, 
                cocktailUrl, em);
        Element tableElement = document2.getElementsByClass("infobox")
                .isEmpty() ?
        ...
        for (Element liElement : liElements2){
            ...
            String ingredient = liElement.getElementsByTag("a").isEmpty() ?
                    "" :
                    liElement.getElementsByTag("a").get(0).text()
                            .toLowerCase();
            if (!ingredient.equals("")){
                TIngredient ingredientForCrossTable = 
                   insertIngredient(ingredient, em);
                insertCocktailXIngredient
                   (cocktailForCrossTable,ingredientForCrossTable, em);
   ...
   private static TCocktail insertCocktail(String CocktailName,
           String CocktailUrl,EntityManager em){
   ...
   private static TIngredient insertIngredient(String ingredientName,
           EntityManager em){
       
       Query query = em.createQuery("SELECT t FROM TIngredient t");
       List<TIngredient> ingredientList = query.getResultList();
       for (Iterator<TIngredient> it = ingredientList.iterator();
               it.hasNext();){
           TIngredient ingredientFromTable = it.next();
           
           if (ingredientName.trim().toLowerCase().equalsIgnoreCase
        (ingredientFromTable.getName())){
   ...
   private static void insertCocktailXIngredient(TCocktail cocktail,
           TIngredient ingredient, EntityManager em ){
       TypedQuery<TCocktailXIngredient> query = em.createQuery(
            "SELECT t FROM TCocktailXIngredient t WHERE t.cocktail.cocktailId = :cocktailId AND t.ingredient.ingredientId = :ingredientId",
               TCocktailXIngredient.class);
       query.setParameter("cocktailId", cocktail.getCocktailId());
       query.setParameter("ingredientId", ingredient.getIngredientId());
       if(query.getResultList().size() == 0){
           TCocktailXIngredient cocktailXIngredient = new 
                TCocktailXIngredient();
           cocktailXIngredient.setCocktail(cocktail);
           cocktailXIngredient.setIngredient(ingredient);
           cocktailXIngredient.setId(new CocktailIngredientId
                 (cocktail.getCocktailId(), ingredient.getIngredientId()));
           em.persist(cocktailXIngredient);
           em.flush();
           System.out.println("Vložen koktejl a ingredience "
                   +cocktail.getName()+" / "+ingredient.getName());       
       }
    }       
   }
